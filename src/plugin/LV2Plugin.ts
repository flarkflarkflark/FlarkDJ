...
export interface LV2PluginInfo {
  ...
  license: string;
  version: string;
  pluginClass: string; // e.g., 'Filter', 'Dynamics', 'Reverb'
}
...
export class LV2Plugin {
  private wrapper: PluginWrapper;
  private info: LV2PluginInfo;
  private ports: LV2Port[] = [];
  ...
  private parameterNameToSymbol(name: string): string {
  ...
  getExtensionData(uri: string): any {
    switch (uri) {
      case 'http://lv2plug.in/ns/ext/state#interface':
        return {
          save: () => this.saveState(),
          restore: (state: string) => this.loadState(state)
        };
...
`;
  ...
  generateTTL(): string {
    const portDescriptions = this.ports.map((port, _idx) => {
      ...
      if (port.min !== undefined) portDesc += ` ;\n        lv2:minimum ${port.min}`;
      if (port.max !== undefined) portDesc += ` ;\n        lv2:maximum ${port.max}`;
      if (port.default !== undefined) portDesc += ` ;\n        lv2:default ${port.default}`;
      if (port.unit) portDesc += ` ;\n        unit:unit unit:${port.unit}`;
...
export function createLV2Plugin(sampleRate: number): LV2Plugin {
